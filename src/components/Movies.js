...
class Movies extends Component {
  ...
  getFeaturedMovies() {
    axios
      .get("https://api.themoviedb.org/3/trending/all/week?api_key=" + TMDBKey)
      .then(response => {
        ...
        return (
          ...
          <div className="searchResult">
            <Link
              ...
              onClick={this.props.setMovie.bind(this, movie)}
            >
              <img
                key={poster}
                src={poster}
              ...
              {name.length <= 50 && <p className="searchResultTitle">{name}</p>}
              {movie.release_date !== undefined && (
                <div>
                  <p className="searchResultDate">
                    {months[parseInt(movie.release_date.substring(5, 7)) - 1] +
                      " " +
      ...
      moviePosters = this.state.featuredMovies.map(movie => {
        ...
        return (
          ...
          <div className="featuredMovieContainer">
            <Link
              to={"/details/" + mediaType + "/" + movieId + "/" + movieTitle}
              style={{ textDecoration: "none" }}
              onClick={this.props.setMovie.bind(this, movie)}
            >
              ...
              {movie.release_date !== undefined && (
                <div>
                  <p className="featuredMovieReleaseDate">
                    (
                    {months[parseInt(movie.release_date.substring(5, 7)) - 1] +
                      " " +
                      movie.release_date.substring(8) +
    ...
    var featuredCarousel = this.state.featuredMovies.map(movie => {
      ...
      if (counter > 5) {
        ...
        return (
          <Carousel.Item>
            <Link
              to={"/details/" + mediaType + "/" + movie.id + "/" + movieTitle}
              style={{ textDecoration: "none" }}
              onClick={this.props.setMovie.bind(this, movie)}
            >
              <img
            ...
            <Carousel.Caption className="carouselText">
              {movie.release_date !== undefined ? (
                <div>
                  <p>
                    {movieTitle} <br />({movie.release_date.substring(0, 4)})
                  </p>
                </div>
              ) : (
                <div>
      ...
      <div className="App">
        ...
        <div id="wrapper">
          {this.state.movies.length > 0 && (
            <div className="flexParent">{moviePosters}</div>
          )}
          {this.state.movies.length === 0 &&
