...
class Details extends Component {
    ...
    if (this.props.location !== prevProps.location) {
      if (url[1] === "home") {
        ...
        this.props.setSearch.bind(
          this,
          decodeURIComponent(url[1].substring(7, url[1].length))
        );
      }
    }
  }
    ...
    axios
      ...
      .then(response => {
        ...
        this.setState(
          ...
          this.getReviews(result, url[3]),
          this.getTrailerKeys(id, mediaType)
        );
      });
  ...
  getRatings(movie) {
    ...
    if (movie.imdb_id === undefined) {
      axios
        .get("http://www.omdbapi.com/?t=" + movie.name + "&apikey=" + OMDBKey)
        .then(response => {
      ...
      axios
        .get(
          "http://www.omdbapi.com/?i=" + movie.imdb_id + "&apikey=" + OMDBKey
        )
        .then(response => {
          let ratings = response.data.Ratings;
          let rated = response.data.Rated;
  ...
  saveToDb(id, search) {
    ...
    search.forEach(element => {
      vid_info.push({
        ...
        channel_id: element.snippet.channelTitle
      });
    });
  ...
  getReviews(movie, id) {
    ...
    if (this.state.mediaType === "person") return;
  ...
  render() {
    ...
    if (
      (cast.length === 0 || ratings === undefined) &&
          ...
          <div key={i}>
            <div style={{ display: "flex", flexDirection: "row" }}>
              <Link
                ...
                to={"/details/person/" + actor.id + "/" + actor.name}
                onClick={this.props.changeToActor.bind(this, actor.id)}
              >
                <div style={{ padding: 3 }}>
                ...
                {this.state.mediaType === "movie" && (
                  ...
                  >
                    <img
                      ...
                      src={require("../icons/imdbStar.png")}
                    />
                    <p
                      style={{
                        color: "white",
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      {rating.Value.substring(0, rating.Value.indexOf("/"))}
                    </p>
                  </div>
                  <p
                    style={{
        ...
        if (credit.job === "Director") {
          return (
            <Link
              ...
              onClick={this.props.changeToMovie.bind(this, credit.credit_id)}
            >
              <div style={{ padding: 3, paddingBottom: 50 }}>
                ...
                <p style={{ color: "#d3d3d3", fontSize: "1.3vh" }}>
                  {credit.job}
                </p>
              </div>
            </Link>
      ...
      if (person.job === "Director") {
        return (
          <Link
            ...
            to={"/details/person/" + person.id + "/" + person.name}
            onClick={this.props.changeToActor.bind(this, actor.id)}
          >
            <p
              style={{
        ...
        <div id="wrapper" style={{ display: "flex", flexDirection: "row" }}>
          <div id="detailsContainer" style={{ display: "flex" }}>
              ...
              <div className="detailsOverview">
                <div id="ratingsContainer" style={{ display: "flex" }}>
                  ...
                  {this.state.mediaType === "movie" && (
                    <div>
                      <p style={{ color: "white", fontSize: "3vh" }}>
                        {movie.title + " (" + releaseDate.substring(0, 4) + ")"}
                      </p>
                    </div>
                  )}
                ...
                {this.state.mediaType === "movie" && (
                  <div>
                    <div className="info">
                      ...
                      <p
                        id="release"
                        style={{ color: "#d3d3d3", paddingRight: 20 }}
                      >
                        {months[
                          parseInt(movie.release_date.substring(5, 7)) - 1
                        ] +
                          " " +
                      ...
                      <p style={{ color: "#d3d3d3", paddingRight: 20 }}>
                ...
                {this.state.showReviews && (
                  <div style={{ width: "100%", padding: 5 }}>
                    <div id="slider" style={{ margin: 0, padding: 10 }}>
                      {resultList.length > 0 && (
                        <Slider {...resultSettings}>{resultList}</Slider>
                      )}
                    </div>
                  </div>
                )}
                  ...
                  <div style={{ width: "100%", padding: 5 }}>
                    <div id="slider" style={{ margin: 0, padding: 10 }}>
                      {cast.length > 0 && this.state.mediaType !== "person" && (
                        <Slider {...castSettings}>{castList}</Slider>
                      )}
                    </div>
                  </div>
                )}
                {this.state.showTrailers && (
                  <div style={{ width: "100%", padding: 5 }}>
                    <div id="slider" style={{ margin: 0, padding: 10 }}>
                      {trailers.length > 0 &&
                        this.state.mediaType !== "person" && (
                          <Slider {...resultSettings}>{trailerList}</Slider>
                        )}
                    </div>
                  </div>
                )}
              ...
              >
                ...
                <div style={{ width: "100%", padding: 5 }}>
                  ...
                  <div id="slider" style={{ margin: 0, padding: 0 }}>
                    <Slider {...castSettings}>{castAndCrewList}</Slider>
                  </div>
                </div>
              </div>
            )}
            ...
            >
              <Modal.Header
                closeButton
                style={{ backgroundColor: "#393f4c", color: "white" }}
              >
                {this.state.mediaType === "movie" ? (
            ...
            >
              ...
              >
                {this.state.mediaType === "person" ? (
                  <img
                    style={{ maxHeight: "70vh", margin: "auto" }}
                    src={
                      "https://image.tmdb.org/t/p/w600_and_h900_bestv2" +
                      this.state.movie.profile_path
                    }
                  />
                ) : (
                  <img
